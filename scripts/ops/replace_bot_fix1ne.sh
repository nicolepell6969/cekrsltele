...
echo "==> Backup bot.js lama (jika ada)"
cp -f bot.js bot.js.bak_fix1ne_$(date +%Y%m%d-%H%M%S) 2>/dev/null || true

echo "==> Tulis bot.js baru (unified + fix /cek 1 NE)"
cat > bot.js <<'JS'
...
echo "==> Pastikan .env memiliki ADMIN_IDS"
if [ -f .env ]; then
...
echo "==> Install deps yang diperlukan"
npm i @whiskeysockets/baileys qrcode-terminal puppeteer@^22 --no-audit --no-fund

echo "==> Commit & push"
git add bot.js .env package.json package-lock.json 2>/dev/null || true
git commit -m "fix: robust /cek 1 NE (always reply) + seeded ADMIN_IDS; WA controls intact" || true
...
echo "✅ Selesai. Uji:"
echo "  /cek SBY-PRMJ-OPT-H910D        (1 NE)"
echo "  /cek SBY-GDK-EN1-H8M14 SBY-BDKL-OPT-H910C  (2 NE)"
