...
cp -f bot.js bot.js.bak_fix_$(date +%Y%m%d-%H%M%S) 2>/dev/null || true

echo "==> Tulis ulang bot.js (versi stabil: /cek 1 NE & 2 NE, parsing teks, WA QR image)"
cat > bot.js <<'JS'
...
echo "==> npm install dep WA QR bila belum"
npm i -s @whiskeysockets/baileys qrcode qrcode-terminal >/dev/null 2>&1 || true
