...
// ---- OVERRIDE /help (safe single handler) ----
try {
  const HELP_TEXT = [
    '📋 *Perintah Utama*',
  ...
  // Menangani /help atau /help@NamaBot
  bot.onText(/^\/help(?:@[\w_]+)?$/i, (msg) => {
    bot.sendMessage(msg.chat.id, HELP_TEXT, { parse_mode: 'Markdown' }).catch(()=>{});
  });
} catch (e) {
  console.error('help override err:', e && e.message ? e.message : e);
}
// ---- END OVERRIDE /help ----
