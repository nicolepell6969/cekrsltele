...
BRANCH="main"
SYSTEMD_SERVICE="cekrsltele"   # jika pakai systemd; kosongkan untuk skip
PM2_NAME="cekrsltele"          # jika pakai PM2; kosongkan untuk skip
# =========================================

echo "==> Masuk ke repo: $REPO_DIR"
cd "$REPO_DIR"

# Pastikan kita di branch target
...
# Config identitas git jika belum ada
git config user.name >/dev/null 2>&1 || git config user.name "alna"
git config user.email >/dev/null 2>&1 || git config user.email "akupro2307@gmail.com"
...
echo "==> Commit & push…"
git add -A
git commit -m "feat: ringkas output 2-sisi + preview & tombol 'Tampilkan semua'"
git branch -M "$BRANCH"
git push -u origin "$BRANCH"

# Restart bila ada systemd/PM2
...
if [[ -n "${PM2_NAME}" ]]; then
  ...
  command -v pm2 >/dev/null && pm2 restart "${PM2_NAME}" || true
fi

echo "✅ Selesai. File diperbarui & dipush ke GitHub."
